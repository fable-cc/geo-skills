...
from config import (
    OPENAI_API_KEY as _CFG_API_KEY,
    OPENAI_BASE_URL as _CFG_BASE_URL,
    LLM_MODEL as _CFG_MODEL,
    GEO_DEFAULT_PLATFORM as _CFG_PLATFORM,
    GEO_DEFAULT_BRAND as _CFG_BRAND,
    GEO_TIMEOUT as _CFG_TIMEOUT,
)
...
def call_llm_stream(messages: list[dict], config: dict) -> typing.Generator[str, None, None]:
...
def call_llm_with_retry(messages: list[dict], config: dict) -> str:
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            last_error = RuntimeError(f"API 返回格式异常: {e}")  # type: ignore[assignment]
            if attempt < max_retries:
                wait = 2 ** attempt
                print(f"\n  重试 {attempt + 1}/{max_retries}（{wait}s 后）...", file=sys.stderr)
                time.sleep(wait)
...
def count_structural_elements(text: str) -> dict[str, int]:
    """统计文本中的结构化元素。"""
    import re
    return {
...
def print_summary(original: str, rewritten: str, platform: Optional[str]) -> None:
    """打印改写摘要。"""
...
def main():
    ...
    # 品牌实体
    parser.add_argument("--brand", default=_CFG_BRAND or None,
                        help="指定品牌实体名称，自动注入正文（如 '景一·寓言城堡'）")
