...
def parse_score_from_text(text: str) -> dict[str, int]:
...
def parse_file_info(file_path: str) -> dict[str, str | int] | None:
...
示例:
  ...
  %(prog)s --stats                                  # 汇总统计
  %(prog)s --trend                                  # 趋势图
  %(prog)s --export csv                      # 导出 CSV (UTF-8 BOM)
  %(prog)s --export json                     # 导出 JSON 数组
        """,
    )
